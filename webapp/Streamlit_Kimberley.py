...
author = st.sidebar.text_area("Find the best book by your favourite author!", "Type author name here...")
def author_high_rating(author_input):
    highest_rating = (df[(df == author_input).any(axis=1)]['minmax_norm_ratings'].max())
    title = df.loc[(df.Author == author_input) & (df['minmax_norm_ratings'] == highest_rating), 'Title']
    print(title)
if st.sidebar.button("Submit"):
    st.dataframe(author_high_rating(author))
